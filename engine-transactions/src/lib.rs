...
impl NormalizedEthTransaction {
    ...
    pub fn intrinsic_gas(&self) -> Result<u64, Error> {
        let config = aurora_engine_evm::config();
...
fn init_code_cost(config: &aurora_engine_evm::Config, data: &[u8]) -> Result<u64, Error> {
    // As per EIP-3860:

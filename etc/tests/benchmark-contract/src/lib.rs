...
pub unsafe extern "C" fn cpu_ram_soak_test() {
    ...
    unsafe { log_utf8(msg.len() as u64, msg.as_ptr() as u64) };
}

fn read_input() -> u32 {
    const REGISTER_ID: u64 = 1;
    unsafe { input(REGISTER_ID) };
    let input_len = unsafe { register_len(REGISTER_ID) };
    assert_eq!(input_len, 4);
    let buf = [0u8; 4];
    unsafe { read_register(REGISTER_ID, buf.as_ptr() as u64) }

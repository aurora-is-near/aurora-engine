...
pub struct AddressMoreThan20 {
    pub info: TransactonTestInfo,
    pub result: HashMap<String, Result>,
    pub txbytes: String
}

impl AddressMoreThan20 {
    pub fn new() -> Self {
        let json_str = read_file("TransactionTests/ttAddress/AddressMoreThan20.json".to_string());
        let tt: TransactionTestJson = serde_json::from_str(&json_str).unwrap();
        let input = tt.AddressMoreThan20.get("AddressMoreThan20").unwrap();
        AddressMoreThan20 {
...
pub struct TransactionTestJson {
    #[serde(flatten)]
    pub AddressMoreThan20: HashMap<String, TransactionTest>,
}

...
pub struct AddressMoreThan20PrefixedBy0 {
    pub info: TransactonTestInfo,
    pub result: HashMap<String, Result>,
    pub txbytes: String
}

impl AddressMoreThan20PrefixedBy0 {
    pub fn new() -> Self {
        let json_str = read_file("TransactionTests/ttAddress/AddressMoreThan20PrefixedBy0.json".to_string());
        let tt: TransactionTestJson = serde_json::from_str(&json_str).unwrap();
        let input = tt.AddressMoreThan20PrefixedBy0.get("AddressMoreThan20PrefixedBy0").unwrap();
...
pub struct TransactionTestJson {
    #[serde(flatten)]
    pub AddressMoreThan20PrefixedBy0: HashMap<String, TransactionTest>,
}

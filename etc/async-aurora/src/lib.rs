...
pub trait Aurora {
    ...
    fn submit(&mut self, #[serializer(borsh)] input: Vec<u8>) -> Promise<SubmitResult>;
}
...
#[near_bindgen]
impl AsyncAurora {
    pub fn submit(&self, #[serializer(borsh)] args: AsyncAuroraSubmitArgs) -> Promise {
        Promise::new(args.silo_account_id)
            .function_call(b"submit".to_vec(), args.input, 0, args.submit_gas)

...
fn test_eth_transfer_success() {
    ...
    test_utils::validate_address_balance_and_nonce(&runner, dest_address, 0.into(), 0.into());
    ...
    test_utils::validate_address_balance_and_nonce(
        &runner,
        source_address,
        (INITIAL_BALANCE - TRANSFER_AMOUNT).into(),
...
fn test_eth_transfer_insufficient_balance() {
    ...
    let transaction = test_utils::create_eth_transaction(
        Some(dest_address),
        (2 * INITIAL_BALANCE).into(), // trying to transfer more than we have
        vec![],
        Some(runner.chain_id),
        &source_account,
    );

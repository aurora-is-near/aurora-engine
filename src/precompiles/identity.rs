...
mod tests {
    ...
    fn test_identity() {
        ...
        let res = Identity::run(&input[0..2], Some(18), &new_context())
            .unwrap()
            .output;
        assert_eq!(res, expected);

        let expected = input.to_vec();
        let res = Identity::run(&input, Some(18), &new_context())
            .unwrap()
            .output;
        assert_eq!(res, expected);

        // gas fail
        let res = Identity::run(&input[0..2], Some(17), &new_context());
        ...
        let res = Identity::run(&input, Some(21), &new_context())
            .unwrap()
            .output;
